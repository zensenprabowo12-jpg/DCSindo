...
const dataContent = fs.readFileSync('client/src/lib/data.ts', 'utf8');

// Extract products array dari file TypeScript
// Cari dari "export const products: Product[] = [" sampai akhir array
const productsMatch = dataContent.match(/export const products: Product\[\] = (\[[\s\S]*\]);/);
if (!productsMatch) {
  console.error('❌ Tidak dapat menemukan array products dalam file!');
  process.exit(1);
}

// Parse JSON (dengan menghapus trailing semicolon dan comments)
let productsJSON = productsMatch[1];

// Hapus comments (// dan /* */)
productsJSON = productsJSON.replace(/\/\*[\s\S]*?\*\//g, ''); // Multi-line comments
productsJSON = productsJSON.replace(/\/\/.*/g, ''); // Single-line comments

// Parse JSON
let products;
try {
  products = eval('(' + productsJSON + ')'); // Menggunakan eval karena ini bukan pure JSON
  console.log(`✅ Berhasil parse ${products.length} produk`);
} catch (error) {
...
console.log(`   - addon_detailed_specs.csv: ${addonDetailedSpecsData.length} rows`);
console.log('\n📝 Langkah selanjutnya:');
console.log('   1. Buat database dan tabel di phpMyAdmin');
console.log('   2. Import CSV files sesuai urutan (products dulu, baru yang lain)');
console.log('   3. Lihat file database-structure.sql untuk struktur tabel yang disarankan');
