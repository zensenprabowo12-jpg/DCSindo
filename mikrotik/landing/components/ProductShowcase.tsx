  ...
  return (
    <div className="relative min-h-screen flex items-center overflow-hidden">
      ...
      <div
        ...
        style={{ padding: "clamp(40px, 6vh, 80px) clamp(16px, 3vw, 32px)" }}
      >
        {/* Header */}
        ...
        <div className="max-w-6xl mx-auto grid lg:grid-cols-2 items-center" style={{ gap: "clamp(24px, 4vw, 40px)" }}>
            ...
            >
              ...
              <ul style={{ margin: "0 0 clamp(12px, 2vh, 20px)", padding: 0, listStyle: "none", display: "flex", flexDirection: "column", gap: "clamp(6px, 1vh, 10px)" }}>
                {active.bullets.map((b, i) => (
                  <li key={`${active.key}-${i}`} style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
          ...
          <div className="flex flex-col items-center">
            ...
            >
              ...
              <div className="flex items-center justify-center" style={{ marginTop: "clamp(12px, 2vh, 24px)", gap: "clamp(6px, 1vw, 10px)", position: "relative", zIndex: 1 }}>
                {products.map((p) => (
                  <SelectorButton key={p.key} label={`Product ${p.key}`} isActive={p.key === activeKey} isDark={isDark} onClick={() => setActiveKey(p.key)} />
                ))}
              </div>
            </div>
