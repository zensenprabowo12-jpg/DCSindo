...
export default function MikrotikDcsStoreProductDetail() {
  ...
  useEffect(() => {
    if (!match) { setLoading(false); setErr(null); return; }
    if (Number.isNaN(id) || id < 1) { setErr("Produk tidak valid"); setLoading(false); return; }
    let c = true;
    void (async () => {
  ...
  const categoryName = (
    (d as any)?.nama_kategori || (d as any)?.category || ""
  ).toString().toLowerCase();

  const backgroundImage =
    CATEGORY_BACKGROUNDS[categoryName] || "/images/category-bg/default.jpg";
  ...
  const backHref = categoryName
    ? `/mikrotik/categories/${encodeURIComponent(categoryName)}`
    : "/mikrotik/categories";

  if (!match) {
    return (
      <Layout>
        <div style={{ padding: "80px 24px", textAlign: "center", color: "rgba(255,107,53,0.8)", fontFamily: "monospace", fontSize: 14 }}>
          URL tidak valid
        </div>
      </Layout>
    );
    ...
    return (
      <Layout>
        <div style={{ padding: "80px 24px", textAlign: "center" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 10, color: "rgba(232,228,220,0.50)", fontSize: 13, fontFamily: "monospace", letterSpacing: "0.1em" }}>
            <span style={{ width: 8, height: 8, borderRadius: "50%", background: "#FF6B35", animation: "pulse 1.2s infinite" }} />
            Memuat…
          </div>
  ...
  if (err || !d) {
    return (
      <Layout>
        <div style={{ padding: "80px 24px", textAlign: "center" }}>
          <p style={{ color: "rgba(255,107,53,0.85)", fontSize: 14, fontFamily: "monospace", marginBottom: 20 }}>
            {err ?? "Tidak ditemukan"}
          </p>
          <BackButton href="/mikrotik/categories" label="← Kembali ke Categories" />
        </div>
      </Layout>
    );
  }
      ...
      >
        ...
        <div
          style={{
            position: "absolute",
            inset: 0,
            background: isDark ? "rgba(10,10,15,0.78)" : "rgba(28,20,14,0.72)",
        ...
        <div style={{ position: "absolute", inset: 0, pointerEvents: "none", display: "flex", justifyContent: "center" }}>
          <div
            style={{
              ...
              filter: "blur(90px)", marginTop: "-5vw",
            }}
          />
        </div>
        ...
        <div style={{ position: "relative", zIndex: 10 }}>
          <div style={{ maxWidth: 1280, margin: "0 auto", padding: "48px 24px 80px" }}>
            ...
            {/* Back button — arahkan ke kategori produk ini */}
            <div style={{ marginBottom: 28 }}>
              <BackButton href={backHref} label="← Kembali ke Categories" />
            </div>
